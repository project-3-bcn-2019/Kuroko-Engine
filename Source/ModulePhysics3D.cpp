...
PhysBody * ModulePhysics3D::AddBody(ComponentPhysics* parent, collision_shape shape, bool is_environment)
{
	...
	{
	case COL_CUBE:
		colShape = new btBoxShape(btVector3(parent->bounding_box.Size().x*0.5, parent->bounding_box.Size().y*0.5, parent->bounding_box.Size().z*0.5));
		break;
	case COL_CYLINDER:
		colShape = new btCylinderShape(btVector3(parent->bounding_box.Size().x*0.5, parent->bounding_box.Size().y*0.5, parent->bounding_box.Size().z*0.5));
		break;
	default:
		colShape = new btBoxShape(btVector3(parent->bounding_box.Size().x*0.5, parent->bounding_box.Size().y*0.5, parent->bounding_box.Size().z*0.5));
		break;
	...
	btRigidBody::btRigidBodyConstructionInfo rbInfo(1.0, myMotionState, colShape,localInertia);//MASS SHOULD BE 0 BUT 1 WORKS SEND HELP
	btRigidBody* body = new btRigidBody(rbInfo);
